...
loadDummyCertificate()
{
  static shared_ptr<Certificate> cert = nullptr;
  if (cert == nullptr) {
    arrayStream is(reinterpret_cast<const char*>(DUMMY_CERT), sizeof(DUMMY_CERT) - 1);
    cert = io::load<Certificate>(is, io::BASE64);
    if (cert == nullptr) {
      throw std::runtime_error("Failed to parse embedded dummy certificate");
    }
  }
  return cert;
}
...
{
  return loadDummyCertificate()->getPublicKey();
}
