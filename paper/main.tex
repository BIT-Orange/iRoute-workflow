...
Separately, the repository also contains a cache-enabled SANR workflow (\texttt{CS\_SIZE}=512) for cache-behavior studies; figures produced from that workflow are treated as cache-enabled SANR results rather than routing-only evidence.
...
Increasing $M$ expands centroid coverage and generally improves domain-level coverage, while increasing $K$ increases the likelihood that a relevant domain is probed at the cost of more discovery traffic.
...
In the current repository, the archived figure bundle for Figs.~\ref{fig:accuracy_overhead} and~\ref{fig:retrieval_cdf} comes from the cache-enabled SANR workflow; until rerun under the cache-disabled paper-grade suite, these plots should be read as cache-enabled workflow evidence rather than pure routing-only evidence.
...
Because the archived repository bundle for Fig.~\ref{fig:retrieval_cdf} is cache-enabled, the plotted CDF reflects the joint effect of discovery and cache participation.
It should therefore not be interpreted as a strict cache-disabled routing-only latency comparison, and Exact-NDN remains a syntactic reference rather than an apples-to-apples intent-discovery baseline when stronger naming knowledge is assumed.
...
Fig.~\ref{fig:recovery} plots success rate over time for these scenarios and reports $t_{95}$ recovery time.
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Success rate over time under churn and failures.}
    \label{fig:recovery}
\end{figure}
...
Link failures test whether discovery can reroute and maintain acceptable success rates when shortest paths disappear.
